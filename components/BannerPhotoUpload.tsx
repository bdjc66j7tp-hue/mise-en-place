...
export default function BannerPhotoUpload({ currentBannerUrl, onUploaded }: Props) {
  ...
  async function handleSaveCrop() {
    ...
    try {
      ...
      const response = await fetch('/api/upload-banner-photo', { method: 'POST', body: formData })
      const data = await response.json()
      if (data.banner_photo_url) {
        onUploaded(data.banner_photo_url)
        setEditingSrc(null)
      } else {
    ...
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.92)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '0.5px solid rgba(151,196,89,0.3)' }}>
          ...
          <div style={{ fontFamily: 'Georgia, serif', fontSize: '15px', fontStyle: 'italic', color: '#C0DD97' }}>Banner</div>
        ...
        <div style={{ position: 'relative', flex: 1, background: '#000' }}>
          <Cropper image={editingSrc} crop={crop} zoom={zoom} aspect={5 / 2} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={onCropComplete} objectFit="contain" showGrid={false} />
        </div>
  ...
  return (
    <div>
      <div style={{ width: '100%', aspectRatio: '5 / 2', background: '#3B6D11', borderRadius: '10px', overflow: 'hidden', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '0.5px solid #C0DD97' }}>
        {currentBannerUrl ? (
          <img src={currentBannerUrl} alt="Banner" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#97C459" strokeWidth="1.5">
            <rect x="3" y="5" width="18" height="14" rx="2"/>
            <circle cx="8" cy="10" r="2"/>
            <path d="M3 16l5-4 4 3 3-4 6 5"/>
          </svg>
        )}
      </div>
      <label htmlFor="banner-photo-input" style={{ display: 'inline-block', background: 'white', color: '#3B6D11', borderRadius: '8px', padding: '8px 14px', fontSize: '12px', fontWeight: '500', cursor: 'pointer', border: '0.5px solid #C0DD97' }}>
        {currentBannerUrl ? 'Change banner' : 'Add banner'}
      </label>
      <input id="banner-photo-input" type="file" accept="image/*" onChange={handleFilePick} style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px', overflow: 'hidden', clip: 'rect(0,0,0,0)', border: 0 }} />
